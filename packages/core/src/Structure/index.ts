...
function getFieldsUsedForChart(olliVisSpec: OlliVisSpec): string[] {
  switch (olliVisSpec.type) {
    case 'facetedChart':
      return [
        ...new Set([
          olliVisSpec.facetedField,
          ...[...olliVisSpec.charts.values()].flatMap((chart: Chart) => getFieldsUsedForChart(chart)),
        ]),
      ];
    case 'chart':
      return [
        ...new Set(
          (olliVisSpec.axes as Guide[])
            .concat(olliVisSpec.legends)
            .reduce((fields: string[], guide: Guide) => fields.concat(guide.field), [])
        ),
      ];
    default:
...
function nodeToDesc(
  ...
  function _nodeToDesc(
    ...
    const chartTitle = (chart: OlliVisSpec) =>
      chart.title || facetValue ? `titled "${chart.title || facetValue}"` : '';
